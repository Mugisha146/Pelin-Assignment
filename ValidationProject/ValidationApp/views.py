    ...
    if request.method == 'POST':
        ...
        if participant_form.is_valid() and vehicle_formset.is_valid():
            ...
            try:
                ...
                participant_form.add_error(None, e.message_dict)
                return render(request, 'participant_form.html', {'participant_form': participant_form, 'vehicle_formset': vehicle_formset, 'action': 'Edit'})
...
def participant_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        if participant_form.is_valid() and vehicle_formset.is_valid():
            ...
            for vehicle_form in vehicle_formset:
                if vehicle_form.is_valid() and 'delete_vehicle' in vehicle_form.cleaned_data:
                    # You can perform any additional logic before deleting if needed
                    vehicle_form.cleaned_data['delete_vehicle'].delete()
